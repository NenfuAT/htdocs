...
$mysqli->set_charset("utf8mb4"); // セミコロン追加
$xml = simplexml_load_file("http://gigazine.net/news/rss_2.0/"); // セミコロン追加
foreach ($xml->channel->item as $feed) {
    ...
    $stmt->store_result();
    if ($stmt->num_rows == 0) {
        $stmt2 = $mysqli->prepare("INSERT INTO rss_item
        (pub_date, title, description, author, link, guid) VALUES (?,?,?,?,?,?)");
    } else {
...
<head>
    ...
    <link rel="stylesheet" href="style.css"> <!-- 正しいlinkタグを使用 -->
</head>
<body>
<h1>サクッとRSSリーダー 記事一覧</h1>
<ul>
<?php foreach ($rss_list as $rss) { ?>
    <li><a href="#no<?php echo $rss["id"]; ?>">
        <?php echo htmlspecialchars($rss["title"], ENT_QUOTES); ?></a></li>
<?php } ?>
</ul>
<?php foreach ($rss_list as $rss) { ?>
    <a name="no<?php echo $rss["id"]; ?>">
    <h2><a href="<?php echo htmlspecialchars($rss["link"]); ?>">
        <?php echo htmlspecialchars($rss["title"], ENT_QUOTES); ?>
    </a>
    </h2>
    <p><strong>作成: <?php echo htmlspecialchars($rss["author"]); ?>
